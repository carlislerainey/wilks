...
# load data
tidy_fits_df <- read_rds("output/bm-tidy-fits.rds") %>%
  glimpse()
...
# write table to file
k %>% cat(file = "doc/tab/bm-fits.tex")
k %>% as_image(file = "doc/tab/bm-fits-gh.png")
...
ks <- k_df %>% 
  filter(Variable == "Two States Have Nuclear Weapons") %>%
  select(-Variable, -`Percent Change`) %>%
  kable(format = "latex", 
        booktabs = TRUE,
        align = c(rep("l", 1), rep("c", 5)), 
...
# write table to file
ks %>% cat(file = "doc/tab/bm-fits-s.tex")
# create image for gh
ks %>% as_image(file = "doc/tab/bm-fits-s-gh.png")
